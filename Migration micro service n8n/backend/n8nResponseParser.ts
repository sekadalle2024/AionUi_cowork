...
function detectTableType(tableKey: string, tableData: any): 'header' | 'data_array' | 'download' | 'unknown' {
  ...
  if (typeof tableData === 'object' && !Array.isArray(tableData)) {
    ...
    const hasDownloadKeywords = lowerKey.includes('telecharger') || lowerKey.includes('download') ||
      keys.some(k => k.toLowerCase().includes('telecharger'));
    const hasUrls = keys.some(k =>
      typeof tableData[k] === 'string' &&
      (tableData[k].startsWith('http://') || tableData[k].startsWith('https://'))
    );
