...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[E-audit] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[E-audit] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[E-audit] 数据迁移失败:', error);
  }
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    console.warn(`[E-audit] Could not find builtin ${dirPath} directory, tried:`, candidates);
    return candidates[0];
  };
  ...
  if (existsSync(builtinSkillsDir)) {
    try {
      ...
      console.warn(`[E-audit] Failed to copy skills directory:`, error);
    }
  }
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            console.warn(`[E-audit] Source rule file not found: ${sourceRulesPath}`);
            continue;
          }
          ...
          console.warn(`[E-audit] Failed to copy rule file ${ruleFile}:`, error);
        }
      }
    } else {
      ...
      for (const [locale, skillFile] of Object.entries(preset.skillFiles)) {
        try {
          ...
          if (!existsSync(sourceSkillsPath)) {
            console.warn(`[E-audit] Source skill file not found: ${sourceSkillsPath}`);
            continue;
          }
          ...
          console.warn(`[E-audit] Failed to copy skill file ${skillFile}:`, error);
        }
      }
    } else {
...
const cleanupOrphanedHealthCheckConversations = () => {
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[E-audit] Cleaned up ${deletedCount} orphaned health-check conversation(s) on startup`);
    }
  } catch (error) {
    console.warn('[E-audit] Failed to cleanup orphaned health-check conversations:', error);
  }
};

const initStorage = async () => {
  console.log('[E-audit] Starting storage initialization...');
  ...
  try {
    ...
    if (!existingMcpConfig || !Array.isArray(existingMcpConfig) || existingMcpConfig.length === 0) {
      ...
      console.log('[E-audit] Default MCP servers initialized');
    }
  } catch (error) {
    console.error('[E-audit] Failed to initialize default MCP servers:', error);
  }
  ...
  try {
    ...
    console.error('[E-audit] Failed to initialize builtin assistants:', error);
  }
...
export const loadSkillsContent = async (enabledSkills: string[]): Promise<string> => {
  ...
  for (const skillName of enabledSkills) {
    ...
    try {
      ...
      console.warn(`[E-audit] Failed to load skill ${skillName}:`, error);
    }
  }
